...
def load_dms_data(dms_location, reference_file, assay_filename_col="DMS_ID"):
    ...
    for filename in reference_df[assay_filename_col]:
        file_path = Path(dms_location) / f"{filename}.csv"
    ...
    for _ in tqdm.tqdm(range(max_iterations)):
        ...
        for i, row_val in val.iterrows():
            for j, row_train in train.iterrows():
                if (
                    sequence_similarity(row_val["sequence"], row_train["sequence"])
                    > 0.8
                ):  # Threshold
                    val = pd.concat(
                        [val.drop(i), pd.DataFrame([row_train])], ignore_index=True
                    )
                    train = pd.concat(
                        [train.drop(j), pd.DataFrame([row_val])], ignore_index=True
                    )
                    improved = True
                    break
            if improved:
...
def save_splits(splits, output_dir, split_type):
    ...
    split_dir = Path(output_dir) / split_type
    ...
    for assay, (train, val) in splits.items():
        ...
        train_file = train_dir / f"{stem}_train.csv"
        val_file = val_dir / f"{stem}_val.csv"
...
def main(dms_location, reference_file, output_dir):
    ...
    for assay, assay_data in data.items():
        ...
        print(
            f"Random split for {assay} - Train size: {len(train_random)}, Validation size: {len(val_random)}"
        )
    ...
    for assay, assay_data in data.items():
        ...
        print(
            f"Similarity-based split for {assay} - Train size: {len(train_sim)}, Validation size: {len(val_sim)}"
        )
    save_splits(sim_splits, output_dir, "min_similarity")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Build fitness benchmark CV splits")
    parser.add_argument("--dms-dir", type=Path, default=ConfigFitness.ASSAY_DIR)
    parser.add_argument(
        "--reference-file", type=Path, default=ConfigFitness.REFERENCE_FILE
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=ConfigFitness.DATA_DIR / "cv_splits",
    )
    arguments = parser.parse_args()
    main(arguments.dms_dir, arguments.reference_file, arguments.output_dir)
