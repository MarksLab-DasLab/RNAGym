...
from rnagym.s3d.models.utils import Adapter, prepare_msa, run
...
def _chain_data(
    ...
    return {
        asym_id: (molecule, sequences[asym_id])
...
def complete(config: Path) -> bool:
    ...
    return (config.parent / "SUCCESS").is_file() and _output_complete(
        _prediction_path(config)
    )
