...
from rnagym.s3d.models.utils import Adapter, run
...
def _output_complete(output_dir: Path) -> bool:
    ...
    return all(
        ...
        for path in (output_dir / "model_00.pdb", output_dir / "model_00.npz")
    )
...
def _prepare_templates(config: Path) -> None:
    ...
    for specification in _write_inputs(config, target_dir / "input", "multimers"):
        ...
        if hhr.is_file() and atab.is_file():
            continue
        command = (
...
def _write_inputs(config: Path, input_dir: Path, kind: str) -> list[str]:
    ...
    for item in raw["sequences"]:
        ...
        if molecule == "rna":
            ...
            path.write_text(_normalize_msa(msa))
            specifications.append(f"R:{path}")
        elif molecule == "protein":
            path.write_text(af3_chain["unpairedMsa"])
...
def complete(config: Path) -> bool:
    ...
    return _output_complete(_PREDICTION_DIR / kind / config.parent.name / "models")
