...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="Run MXfold2 and process outputs for scoring"
    )

    parser.add_argument(
        "--fasta", required=True, help="Path to FASTA file with RNA sequences"
    )
    parser.add_argument(
        "--output_dir", required=True, help="Directory to store MXfold2 output files"
    )
    parser.add_argument(
        "--output_file",
        default=None,
        help="Output CSV file (default: {output_dir}/mxfold2_scores.csv)",
    )
    parser.add_argument(
        "--reference_data",
        default=None,
        help="Path to reference data CSV file (e.g., DMS reactivity data)",
    )
    parser.add_argument(
        "--performance_file", default=None, help="Path to save performance metrics"
    )
    parser.add_argument(
        "--param",
        default="",
        help="MXfold2 parameter file (default: use built-in parameters)",
    )
    parser.add_argument(
        "--model",
        default="Turner",
    ...
    parser.add_argument(
        "--skip_prediction",
        action="store_true",
        help="Skip running MXfold2 prediction (use existing outputs)",
    )
    parser.add_argument(
        "--clean_ids",
        action="store_true",
        help="Clean sequence IDs (replace '.' with '_')",
    )
...
def process_bpp_file(bpp_file, sequence_length):
    ...
    try:
        ...
        if bpp_matrix.shape != (sequence_length, sequence_length):
            print(
                f"Warning: Matrix shape {bpp_matrix.shape} does not match sequence length {sequence_length}"
            )
...
def main():
    ...
    if not args.skip_prediction:
        ...
        id_mapping = run_mxfold2(
            args.fasta, args.output_dir, args.param, args.model, args.clean_ids
        )
    ...
    if len(bpp_files) == 0:
        print(
            "Error: No MXfold2 output files found. Make sure MXfold2 has been run on your sequences."
        )
        sys.exit(1)
    ...
    print(
        f"Successfully processed {len(seq_ids_processed)} sequences out of {len(sequences)}"
    )
    ...
    if results:
        ...
        if args.reference_data and os.path.exists(args.reference_data):
            ...
            processed_df = process_predictions(
                predictions_df, reference_df, model_type="MXfold2"
            )
...
def process_predictions(predictions, test_data, model_type):
    ...
    merged_data = pd.merge(
        test_data, predictions, on=["sequence_id", "position_id"], how="left"
    )
    merged_data = merged_data[merged_data[f"prediction_{model_type}"].notna()]
    return merged_data.groupby("sequence_id", group_keys=False).apply(
        compute_bins, model_type=model_type
    )
...
def save_performance_metrics(metrics, performance_file):
    ...
    metrics_df.to_csv(
        ...
        mode="a",
        index=False,
        header=not os.path.exists(performance_file),
    )
