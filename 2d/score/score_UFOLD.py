...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="Calculate UFold-style unpaired probabilities for RNA sequences"
    )
    ...
    parser.add_argument(
        "--output_file",
        default=None,
        help="Output CSV file (default: {output_dir}/ufold_scores.csv)",
    )
    parser.add_argument(
        "--reference_data",
        default=None,
...
def main():
    ...
    if os.path.exists(args.output_file):
        ...
        predictions_df = load_data(args.output_file)
    ...
    if args.reference_data and os.path.exists(args.reference_data):
        ...
        processed_df = process_predictions(
            predictions_df, reference_df, model_type="UFold"
        )
...
def process_predictions(predictions, test_data, model_type):
    ...
    return merged_data.groupby("seqID", group_keys=False).apply(
