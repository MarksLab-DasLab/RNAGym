        ...
        if csv_file.endswith(".csv"):
            ...
            for model_name in model_list:
                model_path = os.path.join(
                    model_predictions_folder, model_name, csv_file
                )
                if os.path.exists(model_path):
                    model_df = pd.read_csv(model_path)
                    model_mutation_col = get_mutation_column(model_df)
                    score_col = score_cols_dict[
                        model_name
                    ]  # model_df.columns[-1]  # Assume the last column is the score
                    ...
                    model_df = model_df.drop_duplicates(
                        subset=[mutation_col], keep="first"
                    )
                    ...
                    if original_row_count != merged_row_count:
                        ...
                        print(
                            f"Row count mismatch in {csv_file} after merging {model_name}: Expected {original_row_count}, but got {merged_row_count}"
                        )
                        save = False
                else:
