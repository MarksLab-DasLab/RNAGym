...
def score_variants(
    assay, model, tokenizer, base_dir, results_dir, score_column, batch_size
):
    ...
    for i in tqdm(range(0, len(sequences), batch_size), desc="Processing batches"):
        ...
        for j, seq in enumerate(batch):
            ...
            log_probs = normalized_logits[
                j, torch.arange(valid_length), input_ids[:valid_length]
            ]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model_location", type=str, help="NT Huggingface model to use"
    )
    parser.add_argument(
        "--dms_directory",
        type=str,
    ...
    parser.add_argument(
        "--output_directory", type=str, help="Directory for scored fitness files"
    )
