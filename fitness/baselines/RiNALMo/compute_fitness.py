...
def apply_masked_marginal_scoring(
    wt_sequence, mutated_sequence, positions, model, alphabet, device
):
    tokens = torch.tensor(
        alphabet.batch_tokenize([mutated_sequence]), dtype=torch.int64
    ).to(device)
    total_score = 0
...
def process_single_row(
    row, model, alphabet, device, base_dir, results_dir, score_column
):
    ...
    for _, seq in sequences.iterrows():
        mutation_column = (
            "mutant"
            if "mutant" in sequences.columns
            else "mutation"
            if "mutation" in sequences.columns
            else "mutations"
            if "mutations" in sequences.columns
            else None
        )
        positions = extract_positions(seq[mutation_column])
        logits = apply_masked_marginal_scoring(
            wt_seq, seq["mutated_sequence"], positions, model, alphabet, device
        )
